import os
import pandas as pd

customers_path = "C:/Users/geige/Desktop/DHBW/Bachelorarbeit/project/data/results_pipeline/2/_reference/dedup_hard/output.parquet"
products_path = "C:/Users/geige/Desktop/DHBW/Bachelorarbeit/project/data/results_pipeline/2/_reference/products/output.parquet"
orders_path = "C:/Users/geige/Desktop/DHBW/Bachelorarbeit/project/data/results_pipeline/2/_reference/orders/output.parquet"
output_path = "C:/Users/geige/Desktop/DHBW/Bachelorarbeit/project/data/results_pipeline/2/google_isolated_r3/final/output.parquet"

customers = pd.read_parquet(customers_path)
products = pd.read_parquet(products_path)
orders = pd.read_parquet(orders_path)

orders["customer_id"] = orders["customer_id"].astype("int64")
customers["customer_id"] = customers["customer_id"].astype("int64")
orders["product_id"] = orders["product_id"].astype("int64")
products["product_id"] = products["product_id"].astype("int64")

customers = customers.rename(columns={"country": "country_code"})

merged = orders.merge(
    customers[["customer_id", "country_code"]], on="customer_id", how="inner"
).merge(products[["product_id", "category"]], on="product_id", how="inner")

merged = merged[
    merged["country_code"].notna()
    & merged["category"].notna()
]

merged["country_code_clean"] = merged["country_code"].astype(str).str.strip()
merged["category_clean"] = merged["category"].astype(str).str.strip()

merged = merged[
    (merged["country_code_clean"] != "")
    & (merged["country_code_clean"] != "nan")
    & (merged["country_code_clean"] != "None")
    & (merged["category_clean"] != "")
    & (merged["category_clean"] != "nan")
    & (merged["category_clean"] != "None")
]

merged["country_code"] = merged["country_code_clean"]
merged["category"] = merged["category_clean"]

merged["revenue"] = merged["quantity"] * merged["unit_price_eur"]

result = (
    merged.groupby(["country_code", "category"], as_index=False)
    .agg(total_revenue_eur=("revenue", "sum"), order_count=("order_id", "count"))
    .sort_values(by="total_revenue_eur", ascending=False)
)

os.makedirs(os.path.dirname(output_path), exist_ok=True)
result.to_parquet(output_path, index=False)