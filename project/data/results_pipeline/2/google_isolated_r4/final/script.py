...
output_path = r"C:/Users/geige/Desktop/DHBW/Bachelorarbeit/project/data/results_pipeline/2/google_isolated_r4/final/output.parquet"
...
df = df[df['country_code'].notna() & (df['country_code'] != '')]
df = df[df['category'].notna() & (df['category'] != '')]

df['revenue'] = df['quantity'] * df['unit_price_eur']

grouped = df.groupby(['country_code', 'category'], as_index=False).agg(
    total_revenue_eur=('revenue', 'sum'),
    order_count=('order_id', 'count')
)

grouped = grouped.sort_values(by='total_revenue_eur', ascending=False).reset_index(drop=True)
